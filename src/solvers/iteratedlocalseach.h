...
{
public:
    IteratedLocalSearch(unsigned int numberOfSolutions, unsigned int perturbationStrength, std::shared_ptr<Problem<T>> prob) : GlobalSolver<T>(numberOfSolutions, prob)
    {
        this->perturbationStrength = perturbationStrength;
        this->numberOfAgents = numberOfSolutions;
...
#pragma omp parallel for
        for (int i = 0; i < this->numberOfAgents; i++)
        {
            std::shared_ptr<Solution<T>> sol = this->problem->initialize(solutions[i]);
