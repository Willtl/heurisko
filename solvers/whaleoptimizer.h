...
{
public:
    WhaleOptimizer(int numberOfIndividuals, shared_ptr<Problem<T>> prob) : GlobalSolver<T>(numberOfIndividuals, prob)
    {
...
#pragma omp parallel for
            for (size_t i = 0; i < this->numberOfAgents; i++) {
                for (size_t j = 0; j < this->prob->getDimension(); j++) {
                    ...
                    wolves[i][j] = max(this->prob->getLb()[j], min(X1 + X2 + X3 / 3, this->prob->getUb()[j]));
                }
            }
